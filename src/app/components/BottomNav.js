        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href}
              className={`flex flex-col items-center gap-1 py-2.5 text-[10px] font-medium transition-colors ${isActive ? "text-accent" : "text-slate-400 hover:text-slate-200"}`}>
              <span className={`relative ${isActive ? "text-accent" : ""}`}>
                ...
                {isActive && <span className="absolute -bottom-1.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-accent" />}
              </span>
              {item.name}
            </Link>
          );
