...
export default async function NotificationsPage() {
  ...
  // Paginate: limit to recent 50, newest first.
      ...
      <header className="sticky top-0 z-30 border-b border-line bg-night/90 backdrop-blur-md">
        <div className="mx-auto flex h-16 w-full max-w-lg items-center justify-between px-4">
          <div className="flex items-center gap-2.5">
            <Link href="/" className="flex h-8 w-8 items-center justify-center rounded-lg border border-line bg-night-800 text-slate-300 hover:text-white">←</Link>
              ...
              return (
                <Link key={n.id} href={href}
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {!n.is_read && <span className="shrink-0 h-2 w-2 rounded-full bg-accent" />}
                  </div>
